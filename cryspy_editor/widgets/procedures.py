import numpy
from cryspy_editor.widgets.from_cryspy import calc_sthovl_by_unit_cell_parameters, calc_index_hkl_in_range, calc_unit_cell_parameters_and_u_by_ub
from cryspy_editor.widgets.colors import L_COLOR_TABLE, L_COLOR, transform_color
from cryspy_editor.widgets.function_dictionary import transform_name_to_table_name
...
def procedure_column_general(s_line: str, d_np_table: dict):
    ...
    if len(l_name) < 1:
        raise ValueError(f"For command '{s_command.capitalize():}' the name of the column should be given")

    for name in l_name:
        val, flag, name_in_d = take_val(name, d_np_table)
        if not flag:
            raise ValueError(f"The column '{name:}' should be defined in table to cald '{s_command.capitalize():}'")
        ...
        if s_command == "sum":
            ...
            raise ValueError(f"The command '{s_command.capitalize:}' is not defined in the program.")
...
def procedure_copy_to_clipboard(s_line: str, d_np_table: dict):
    ...
    for i_name, name in enumerate(l_name):
        val, flag, name_in_d = take_val(name, d_np_table)
        if not flag:
            raise ValueError(f"The column '{name:}' should be defined in table to copy to clipboard")
        l_val.append(val)
    ls_out = []
...
def procedure_column_del(s_line: str, d_np_table: dict):
    ...
    if len(l_hh_more)*len(l_hh_less)*len(l_hh_equal) != 2:
        raise ValueError("Inequality function should be only one '>', '<' or '=='")
    l_flag = [flag_more, flag_less, flag_equal]
    l_hh = [_ for flag, _ in zip(l_flag, [l_hh_more, l_hh_less, l_hh_equal]) if flag][0]    
    ...
    if np_flag.size != len(d_np_table[" table_colors"]):
        d_np_table[" table_colors"] = [transform_color(L_COLOR[0]), ]*np_flag.size
    i_diff = 0
...
def procedure_print(s_line: str, d_np_table: dict):
    ...
    for name in l_hh:
        ...
        if not flag:
            raise ValueError(f"The column '{name:}' should be defined in table to print")
        l_name_d.append(name_d)
        l_val.append(val)
    l_command_d = ["", ]*len(l_name_d)
...
def procedure_column_mark(s_line: str, d_np_table: dict):
    ...
    val_r, flag_r = estimate_expression(s_expr_right, d_np_table)
    if not(flag_l and flag_r):
        raise ValueError("Given expression in 'mark' function is not supported")
...
def procedure_equiv_hkl(s_line: str, d_np_table: dict):
    ...
    np_hkl = numpy.stack([d_np_table["H"], d_np_table["K"], d_np_table["L"]], axis=1)
    ...
    d_command = {"H": "", "K": "","L": ""}
    for name, s_command in zip(d_np_table[" table_names"], d_np_table[" table_commands"]):
        del d_np_table[name]
        if name in ["H", "K", "L"]:
            d_command[name] = s_command
        
    d_np_table[" table_names"] = ["H", "K", "L"]
    d_np_table[" table_commands"] = [d_command["H"], d_command["K"], d_command["L"]]
...
def procedure_group_hkl(s_line: str, d_np_table: dict):
    ...
    flag_table = all([name in d_np_table[" table_names"] for name in l_necessary_table]) 
    if not flag_table:
        raise ValueError("The table does not contain columns " + ", ".join(l_necessary_table))
    ...
    for i_c, l_inx_equiv in enumerate(ll_ind_equiv):
        ...
        l_table_colors.extend([colour, ]*len(l_inx_equiv))
...
def procedure_aver_hkl(s_line: str, d_np_table: dict):
    ...
    if not flag_iint and not flag_fsq:
        raise ValueError("The table does not contain columns " + ", ".join(l_iint) + " or " + ", ".join(l_fsq))
    ...
    r_factor_iint_1, r_factor_fsq_1 = 0., 0.
    r_factor_iint_2, r_factor_fsq_2 = 0., 0.
    for l_inx_equiv in ll_ind_equiv:
        ...
        if flag_iint:
            ...
            aver_iint, saver_iint, af_aver_iint = calc_aver_saver(np_iint, np_siint) 
    ...
    d_np_table["H"] = numpy.array([_[0] for _ in l_hkl_new], dtype=np_hkl.dtype)
    d_np_table["K"] = numpy.array([_[1] for _ in l_hkl_new], dtype=np_hkl.dtype)
    d_np_table["L"] = numpy.array([_[2] for _ in l_hkl_new], dtype=np_hkl.dtype)
    l_flag_good = [True, ]*len(l_hkl_new)
    l_flag_bad = [False, ]*len(l_hkl_new)
    d_np_table[" table_colors"] = []
    if flag_iint:
        d_np_table[" table_names"].extend(["Intensity", "sIntensity", "AF_Intensity"])
        d_np_table[" table_commands"].extend(["", "", ""])
        d_np_table["Intensity"] = numpy.array([_[0] for _ in l_aver_iint], dtype=float)
        d_np_table["sIntensity"] = numpy.array([_[1] for _ in l_aver_iint], dtype=float)
        d_np_table["AF_Intensity"] = numpy.array([_[2] for _ in l_aver_iint], dtype=float)
        l_flag_bad = [_[2]>20 or _2 for _, _2 in zip(l_aver_iint, l_flag_bad)]
        l_flag_good = [_[2]<5 and _2 for _, _2 in zip(l_aver_iint, l_flag_good) ]
        if r_factor_iint_2 > 0:
            ...
            if not(s_name in d_np_table[" inline_names"]):
                d_np_table[" inline_names"].append(s_name)
            d_np_table[s_name] = [r_factor_iint, ]
    if flag_fsq:
        d_np_table[" table_names"].extend(["Fsq", "sFsq", "AF_Fsq"])
        d_np_table[" table_commands"].extend(["", "", ""])
        d_np_table["Fsq"] = numpy.array([_[0] for _ in l_aver_fsq], dtype=float)
        d_np_table["sFsq"] = numpy.array([_[1] for _ in l_aver_fsq], dtype=float)
        d_np_table["AF_Fsq"] = numpy.array([_[2] for _ in l_aver_fsq], dtype=float)
        l_flag_bad = [_[2]>20 or _2 for _, _2 in zip(l_aver_fsq, l_flag_bad)]
        l_flag_good = [_[2]<5 and _2 for _, _2 in zip(l_aver_fsq, l_flag_good) ]
        if r_factor_fsq_2 > 0:
            r_factor_fsq = 100* r_factor_fsq_1/r_factor_fsq_2
            s_name = "Agreement Factor for Fsq"
            if not(s_name in d_np_table[" inline_names"]):
                d_np_table[" inline_names"].append(s_name)
            d_np_table[s_name] = [r_factor_fsq, ]
...
def procedure_calc(s_line: str, d_np_table: dict):
    ...
    if s_name == "sthovl":
        ...
        l_necessary_inline_3 = ["Wavelength", ]
        ...
        if len(l_name) < 2 or len(l_name) > 3:
            raise ValueError("For '{s_name:}' the name of y_mod, y_exp, and y_sigma should be given")
    else:
        return
    ...
    if flag_hkl_ucp:
        np_hkl = numpy.stack([d_np_table["H"], d_np_table["K"], d_np_table["L"]], axis=0)
        unit_cell_parameters = numpy.array([d_np_table[name] for name in l_necessary_inline], dtype=float)
        unit_cell_parameters[3:] = numpy.radians(unit_cell_parameters[3:])
        np_sthovl = calc_sthovl_by_unit_cell_parameters(np_hkl, unit_cell_parameters)
    if s_name == "sthovl":
        ...
        flag_inline = all([name in d_np_table[" inline_names"] for name in l_necessary_inline_3])
        if not flag_inline:
        ...
        if len(l_name) == 3:
            np_y_sigma, flag_y_sigma, name_y_sigma = take_val(l_name[2], d_np_table)
            if not flag_y_exp:
                raise ValueError(f"The column {l_name[2]:} is not defined")
        else:
            np_y_sigma = numpy.ones_like(np_y_mod)

        np_chisq = numpy.sum(numpy.power((np_y_mod-np_y_exp)/np_y_sigma, 2))
...
def procedure_generate_hkl(s_line: str, d_np_table: dict):
    ...
    if s_name[0].lower() == "sthovl":
        ...
        sthovl_min = numpy.sin(numpy.radians(sthovl_min))/d_np_table["Wavelength"]
        sthovl_max = numpy.sin(numpy.radians(sthovl_max))/d_np_table["Wavelength"]
    elif s_name[0].lower() == "2theta": # 2theta
        sthovl_min, sthovl_max = float(s_name[1]),float(s_name[2])
        if not "Wavelength" in d_np_table[" inline_names"]:
            raise ValueError("The parameter 'Wavelength' should be defined")
        sthovl_min = numpy.sin(0.5*numpy.radians(sthovl_min))/d_np_table["Wavelength"]
        sthovl_max = numpy.sin(0.5*numpy.radians(sthovl_max))/d_np_table["Wavelength"]
    else: # 2theta
        sthovl_min, sthovl_max = float(s_name[0]),float(s_name[1]),
        if not "Wavelength" in d_np_table[" inline_names"]:
            raise ValueError("The parameter 'Wavelength' should be defined")
        sthovl_min = numpy.sin(0.5*numpy.radians(sthovl_min))/d_np_table["Wavelength"]
        sthovl_max = numpy.sin(0.5*numpy.radians(sthovl_max))/d_np_table["Wavelength"]
    ...
    np_hkl = calc_index_hkl_in_range(sthovl_min, sthovl_max, unit_cell_parameters)
    ...
    d_np_table[" table_colors"] = [transform_color(L_COLOR_TABLE[0]), ]*sthovl.size
...
def procedure_column(s_line: str, d_np_table: dict):
    ...
    if len(d_np_table[" table_names"])==1 and table_name in d_np_table[" table_names"]:
...
def calc_unit_cell_by_ub(d_np_table):
    l_necessary_inline = ["a", "b", "c", "alpha", "beta", "gamma"]
    l_necessary_inline_2 = ["UB_11", "UB_12", "UB_13", "UB_21", "UB_22", "UB_23", "UB_31", "UB_32", "UB_33"]
    flag_inline_2 = all([name in d_np_table[" inline_names"] for name in l_necessary_inline_2])
    if not flag_inline_2:
        raise ValueError("The following parameters should be defined: " + ", ".join(l_necessary_inline))    
    else:
        m_ub = numpy.array([float(d_np_table[name]) for name in l_necessary_inline_2], dtype=float)
    ...
    for i_name, name in enumerate(l_necessary_inline):
        d_np_table[name] = [float(unit_cell_parameters[i_name]), ]
        if i_name in [3, 4, 5]:
            d_np_table[name] = [float(numpy.degrees(d_np_table[name])), ]
    return
                ...
                for f_3 in f_2.split(L_OPERATION[3]):
                    ...
                    for f_4 in f_3.split(L_OPERATION[4]):
                        ...
                        for f_5 in f_4.split(L_OPERATION[5]):
                            val_mod, flag, name_in_d = take_val(f_5, d_np_table)
                            if not flag:
                                return None, flag
                            l_val_mod.append(val_mod)
                        l_val_mod_inv = l_val_mod[::-1]
                        val_power = sequent_func(*l_val_mod_inv, func = lambda x,y: y%x)
                        # val_power, flag, name_in_d = take_val(f_4, d_np_table)
                        # if not flag:
                        #     return None, flag
                        l_val_power.append(val_power)
                    l_val_power_inv = l_val_power[::-1]
                    val_dev = sequent_func(*l_val_power_inv, func = lambda x,y: x**y)
